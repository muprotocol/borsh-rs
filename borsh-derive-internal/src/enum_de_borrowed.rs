...
pub fn enum_de_borrowed(input: &ItemEnum, cratename: Ident) -> syn::Result<TokenStream2> {
                ...
                for field in fields.unnamed.iter() {
                    if contains_skip(&field.attrs) {
                        ...
                        variant_header.extend(
                            quote! { #cratename::BorshBorrowedDeserialize::deserialize(buf)?, },
                        );
                    }
                }
                variant_header = quote! { ( #variant_header )};
    ...
    let variant_idx = quote! {
        let variant_idx: u8 = #cratename::BorshBorrowedDeserialize::deserialize(buf)?;
    };
